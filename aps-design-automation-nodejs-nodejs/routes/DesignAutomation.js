...
router.get('/auth/token', async (req, res, next) => {
    ...
    req.oauth_token = req.oauth_client.getCredentials();
    res.json({ access_token: req.oauth_token.access_token, expires_in: 3600 }).end()
});
...
class Utils {
    ...
    /// </summary>
    static get NickName() {
        return "myautocadtest"//(config.credentials.client_id);
    }
    ...
    static EngineAttributes(engine) {
        ...
        if (engine.includes('Revit'))
            ...
            return ({
                commandLine: '$(engine.path)\\revitcoreconsole.exe  /al "$(appbundles[{0}].path)"',
                extension: 'rvt',
                script: ''
            });
...
router.post('/aps/designautomation/workitems', multer({
    ...
    const outputFileNameOSS = `${new Date().toISOString().replace(/[-T:\.Z]/gm, '').substring(0, 14)}_output_${_path.basename("outputFile.rvt")}`; // avoid overriding  
...
router.post('/aps/callback/designautomation', async /*OnCallback*/(req, res) => {
    ...
    try {
        ...
        try {
            ...
            socketIO.to(req.query.id).emit('onComplete', 'Start to translating output model...');

            //get info of output file in bucket
            const details = await objectsApi.getObjectDetails(bucketKey, req.query.outputFileName, {}, req.oauth_client, req.oauth_token)
            const urn = Buffer.from(details.body.objectId).toString('base64')
                .replace(/\+/g, '-') // Convert '+' to '-'
                .replace(/\//g, '_') // Convert '/' to '_'
                .replace(/=+$/, '');

            //post translating job
            const mdApi = new ForgeAPI.DerivativesApi();

            const job = {
                input: { urn },
                output: { formats: [{ type: 'svf2', views: ['2d', '3d'] }] }
            }; 
            const resp = await mdApi.translate(job, {}, req.oauth_client, req.oauth_token);

            var status = false
            while (true) {
                //tracking the progress. you may need to add a time out
                const translation_resp = await mdApi.getManifest(urn, {}, req.oauth_client, req.oauth_token);
                if (translation_resp.body.status == 'success') {
                    status = true
                    break;
                }
                //notify client about the progress
                socketIO.to(req.query.id).emit('onComplete', 'translation progress...' + translation_resp.body.progress);
            }

            if (status) {
                //when it is done, send urn to client side.
                socketIO.to(req.query.id).emit('onComplete', 'Translation Done...');
                socketIO.to(req.query.id).emit('onComplete', { viewer_urn: urn });
